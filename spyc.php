...
//    if (!$line) {return array();}
  ...
  private function _toType($value) {
    ...
    do {
      ...
      ($last_character != '"' && $last_character != "'")) {break;}
      $is_quoted = true;
    } while (0);
  ...
  private function _inlineEscape($inline) {
    ...
    $i++;
    if ($i > 10){break; /*Prevent infinite loops.*/}
    }
  ...
  private static function startsLiteralBlock ($line) {
    ...
    if (($lastChar != '>' && $lastChar != '|') || (preg_match ('#<.*?>$#', $line))) {return false;}
...
/*    if (preg_match ('#<.*?>$#', $line)) return false;*/
  ...
  private static function greedilyNeedNextLine($line) {
    $line = trim ($line);
    if ((!strlen($line)) || (substr ($line, -1, 1) == ']')) return false;
    if (($line[0] == '[') || (preg_match ('#^[^:]+?:\s*\[#', $line))) return true;
    return false;
  }
  ...
  private function isArrayElement ($line) {
    if ((!$line || !is_scalar($line)) || (substr($line, 0, 2) != '- ')) {return false;}
    if ((strlen ($line) > 3) && (substr($line,0,3) == '---')) {return false;}
    return true;
  }
  ...
  private function isLiteral ($line) {
    if (($this->isArrayElement($line)) || ($this->isHashElement($line))) {return false;}
    return true;
  }
